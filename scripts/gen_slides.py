...
TEMPLATE = ROOT / "report" / "slides" / "video_deck.template.html"
OUTPUT = ROOT / "report" / "slides" / "video_deck.html"
...
def render() -> str:
    ...
    banner = (
        "<!-- GENERATED by scripts/gen_slides.py from report/slides/video_deck.template.html\n"
...
def main() -> int:
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="exit non-zero if the committed deck differs from a fresh render",
    )
    ...
    print(f"wrote {OUTPUT.relative_to(ROOT)} ({slides} slides, {len(rendered)/1024:.0f} KB)")
    return 0
