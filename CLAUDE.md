...
On record: on 2026-08-04 a session pasted a 31-line experiment report here that duplicated
`docs/03-results/SUCCINCT_PQ_PROOF_EXPERIMENT.md`; it was compressed to its rules and a pointer.
Follow the standing duty above **and** this budget — recording context and dumping text are not
the same act.
...
*Regenerated 2026-08-04 17:42 by `scripts/update_claude_context.py`, which only reads files and git metadata — it never builds, tests, or benchmarks, and never estimates a number. Anything it could not parse says (not found).*

### Repository right now

- Branch **`report`** · HEAD a44468a · 2026-08-04 · CLAUDE.md update
- Working tree: 6 modified tracked file(s), 218 untracked path(s) · no upstream tracking branch
- Recent commits:
...
- Last checkpoint in `PROGRESS.md`: Checkpoint — 2026-08-04 — LaBRADOR run: succinct+PQ+zk measured; direction CLOSED
...
- `role_a_air` + `src/bin/bench_role_a.rs` — **built and measured 2026-08-04 (Royce-directed).**
  ...
  **Framings that must not drift:** it works but **at this size does not pay** — larger proof,
  worse total compute, much faster verification; the real finding is the **cost split**, i.e. it
  relocates work onto the prover rather than reducing it, and succinctness is asymptotic while
  one role-A relation is too small to reach it. The verification win lands in the wrong place
  for a two-party swap (each proof is verified once); it is the right profile for the *on-chain*
  question but **does not solve it**. No concrete-security analysis; the three provers' security
  levels are **not** claimed equal; **not wired into the swap**.
  **⚠️ DISQUALIFIED AS π (Royce, 2026-08-04):** eprint 2020/845 §4.1 needs π to *hide* the
  witness — if u₂ learned `r` it could adapt σ̂₁ and take both sides — so **π must be
  ZERO-KNOWLEDGE**, and this STARK is not. It is therefore **not a candidate prover**, and the
  comparison flatters it on an axis where it does not qualify. Never present it as a drop-in.
  The right target is succinct + PQ + **zk** = **LaBRADOR**, which LaZer ships.
...
**Two former future-work items, promoted and RUN 2026-08-04 (Royce).** He was told this breaks
the Meeting-8 freeze and competes with the remaining deliverables, and chose it anyway; that
stands. Quote the write-ups for numbers, never this file.
- **Statement compression** — `ref/test/test_statement_compress.c` (+`{2,3,5}`, in `all`),
  ...
  must hold the contract or nothing is attributable; decisive rows are P3 (base Verify accepts
  the adapted signature) and P4 (Ext recovers the witness). **Result: "the statement is not
  compressible the way the public key is" is CONFIRMED, with a mechanism** — *truncation is
  invisible to the adaptor's own functions (both sides hash whatever statement they were handed)
  and fatal at both boundaries: `base_verify` never sees a statement and recomputes `Az−ct = w+Y`
  with the TRUE `Y`, and Ext's acceptance test IS the exact relation `A s == Y`* — failing at
  every depth, not marginally. The seed candidate passes everything **and lets the receiver
  recover the witness**: the largest available compression is a total break. Hint-style repair is
  net +0 B by construction; lossless 23-bit packing is the baseline to beat. **Do not reopen `Y`
  compression inside this construction; the open question is a different hard relation whose
  statement is smaller by design.**
- **Proof amortisation — measured for BOTH provers; question CLOSED, batching fails on both for
  opposite reasons.** Groth16: `rust/las-swap/src/bin/bench_amortise.rs` +
  `BatchedRelationCircuit` (relation factored into `emit_instance`, shared by single and batched
  circuits, plus a per-batch tamper check, so a batch cannot prove something weaker) →
  `evidence/amortise/latest` (⚠️ three runs that day — **`latest` is the one to quote**),
  write-up `docs/03-results/PROOF_AMORTISATION_EXPERIMENT.md`. **NEGATIVE result:** proof size is
  constant in k so per-swap bytes fall `1/k`, **but bytes were never the bottleneck** — proving
  time dominates; per-swap proving is flat to slightly worse, never better (claim no monotone
  law). **Framing: a statement about *Groth16*, not about batching.** LaZer: batched π —
  `ref/relation_zk_batch.{c,h}` (block-diagonal: k copies of `[I|A'|−I|−A'|0]`, so the batch is
  the conjunction of k copies of the *deployed* statement) + `ref/relation_zk_lazer_batch.{c,h}`
  (the **second and only other** TU that may include `lazer.h`) + committed
  `relation_zk_params_k{2,4,8}.h` ⇐ `scripts/gen_lazer_batch_params.sh` (SageMath at
  `~/micromamba/envs/lazer-sage/bin/sage`), `ref/test/bench_lazer_amortise.c`,
  `scripts/run_lazer_amortise.sh` → `evidence/lazer_amortise/<ts>/`; **k=1 dispatches to the
  COMMITTED `las_pi_params`**. **Result: proof/swap drops substantially but per-swap
  prove+verify gets several times worse** (superlinear work, and the role-A proof already
  dominates config 3) — a genuine trade-off, the wrong way round here. ⚠️ **NOT wired into the
  swap**: deployed prover, KAT and every benchmark of record untouched; batched param sets are
  **not independently reviewed** and no security claim is made about batching.
...
*Cleanup state, verified 2026-08-04:* `report/latex/` is **clean** — the only remaining
mention is the background sentence at `01-introduction.tex:79`, which passes the test below.
...
## What remains (verified 2026-08-04)

1. **Report polish** — word count against the 9,000 ceiling; the frontmatter `\TODO`s in
   `report/latex/report.tex` (student id, prior degrees, acknowledgements) — **Royce only**.
2. **Rewrite the two Chapter 5 future-work bullets the new experiments answered** —
   `Shrink the statement, not the signature` (`05-conclusion.tex:119`) and `Reduce the proof`
   (`:126`) still read as untested directions; both are now measured (see Status).
3. **6–8 minute presentation slides** — Wang's deliverable. `report/slides/` currently holds
   only `stage1_summary.html` (2026-07-25); the deck does not exist yet.
4. **AMHL doc cleanup** in `docs/` under the (a)/(b) test above.

**Done — do not re-queue:** the ML-DSA hint result is folded into Ch. 4 (`04-evaluation.tex`
§"Reference optimisations appear to fight the adaptor identity" now records the corrected
"sufficient route, not a necessary one" claim); the `q≈2^24` bullet is gone from Ch. 5; the
`Adapt`-vs-ECDSA gap is explained (`03-results.tex:322`, detail in
`docs/03-results/LAS-08-performance-measured.md`); both new experiments are RUN with evidence
and write-ups; Meeting-8's transaction breakdown + diagrams, terminology split, Chapter 5
title and functions-not-protocols wording are satisfied.
...
is the default. Royce has overridden it explicitly twice (2026-08-04: the two promoted
future-work items, and the role-A STARK); those stand, but an override must come from him.
When in doubt about order, ask Royce rather than silently reordering.
...
- Deliverable = the three benchmarked configurations ✅. **Metrics: gas → time +
  communication cost**, counting off-chain messages, plus the usability finding (heavy
  pre-transaction computation may need a dedicated PC rather than a phone).
...
- Report rulings applied: evaluation is its own chapter; critical reflection is its own
  section in Chapter 5 (achieved / fell short / would do differently); the rejection figure is
  a **cumulative acceptance curve**, not P(exactly k attempts).
