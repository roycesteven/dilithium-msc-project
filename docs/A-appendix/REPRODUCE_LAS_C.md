...
Companion documents: `docs/02-methodology/FUNCTION_MAP.md` (reuse classification),
`docs/02-methodology/THEORY_IMPL_BRIDGE.md` (paper equation → C line), `docs/LAS.md` (full design),
`docs/paper/LAS_2020_845_NOTATION.md` (notation source of truth). The Rust
counterpart of this guide is `docs/A-appendix/REPRODUCE_LAS_RUST.md`.
...
The whole methodology rests on one decision, recorded in `docs/02-methodology/FUNCTION_MAP.md`:
...
The methodology **mirrors the Rust driver exactly** (since 2026-07-06):
5 repetitions × 500 iterations for the sign-class operations / 1000 for the
verify-class ones (mean ± sample SD), the same fixed pp seed `00..1f` and the
same fixed 33-byte message, a printed per-attempt (rejection-normalised)
diagnostic, and the **run-validity rejection gate**: the attempts/call measured
over the 2 500 timed sign-class calls is hard-checked against the exact
theoretical expectation (`las_expected_attempts` in `ref/las.{c,h}`) within a
5σ statistical tolerance — a run whose restart rate deviates from theory
prints `FAIL` and aborts instead of producing plausible-looking but invalid
evidence. Expect two `rejection gate [...] => OK` lines in every log.
...
4. **Rejection rate** is read directly off `base_attempts`/`las_attempts`:
   ...
   `γ = κ·d·(n+ℓ)` design target of *e* restarts (2020/845 §3.2). The exact
   expectations at the headline setting are Sign **2.71875** and PreSign
   **2.77483** attempts/call (the −1 tighter PreSign bound shifts it by ~2%),
   and every run's gate line proves its own measured rate matches them within
   5σ.
