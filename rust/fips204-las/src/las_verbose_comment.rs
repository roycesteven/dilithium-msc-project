...
/// `las_signature_internal`/`las_presign_internal`; never read by the scheme itself.  Benchmarks
...
/// Shared Sign body, parameterised by the 64-byte mask seed. Mirrors las_signature_internal.
...
/// Deterministic Sign: mask randomness derived from (sk, M). Mirrors las_signature_det.
...
/// C `las_keypair` = `randombytes` + `las_keypair_seed`; RNG injected, Rust idiom).
pub fn las_keypair(
    pp: &LasPp,                    // paper A: pp = A = [I | A']
...
/// Randomised Sign: fresh 64-byte mask seed per call (mirrors C `las_signature`).
