# LAS on Dilithium — extended build / run / reproduce guide

> **This is the extended reproducibility guide.** For the short version with only the
> key commands, see [README.md](README.md). This file has the full evidence pipeline,
> every report-table→log mapping, the figure catalogue, and the optional baselines.
...
Every table reported in the dissertation
...
| `./test/test_serde3` | `serialization_tests.log` | Byte encoding round-trips; all 6720 single-byte flips of a packed signature are rejected at Verify. The wire is `c_tilde ‖ BitPack(z)` (32-byte challenge digest + response `z` packed with the reused FIPS BitPack); the validating decoder rejects malformed pk (coeff≥Q) and sk (code 3) bytes, while `c_tilde`/`z` decode permissively (tamper caught at Verify, upstream-faithful). Also run across the D2/D3/D5-aligned sets (`test_serde_l2/l3/l5`); the packed `z` width is parameter-derived (18 bits at paper/D2, 19 at D3/D5). |
...
(identical to the Rust `las_kat` digest — the cross-language interoperability gate);
...
**one** consistent state (one `setup_public_params`, one key pair, one statement/witness, one
...
the response `z` is ≈99.5% of the signature (6688 of 6720 B at mode 3), the challenge
`c_tilde` a fixed 32 B.
...
```text
ref/setup.{c,h}      shared params + public_params pp=(A,H) + setup_public_params()
ref/las_types.h      the six protocol object types (public/secret key, signature, statement, witness, pre-signature)
ref/relation.{c,h}   hard-relation Gen -> (statement Y = t', witness r')
ref/basesig.{c,h}    Algorithm 1 (base_keygen/base_sign/base_verify); simplified Dilithium-style base signature (no Y; fair-benchmark baseline)
ref/las.{c,h}        Algorithm 2 (las_presign/las_preverify/las_adapt/las_ext) layered on basesig
ref/serialize.{c,h}  byte-level codec (wire = c_tilde ‖ BitPack(z)) + validating decoder; base_verify_packed is in basesig.c
