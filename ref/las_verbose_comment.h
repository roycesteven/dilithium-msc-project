 ...
 * Counts the total number of rejection-loop attempts performed by las_signature,
...
void las_keypair(las_pk *pk, las_sk *sk, const las_pp *pp);

/* Deterministic KeyGen from an explicit 32-byte seed (reproducible KAT vectors). */
void las_keypair_seed(las_pk *pk, las_sk *sk, const las_pp *pp,
                     const uint8_t seed[LAS_SEEDBYTES]);

/* Sign / Verify (ordinary signature; Verify returns 0 on success). */
void las_signature(las_sig *sig, const uint8_t *m, size_t mlen,
 ...
 * validity as las_signature; removes the per-signature RNG (no nonce-reuse risk) and
 * enables reproducible known-answer tests. */
void las_signature_det(las_sig *sig, const uint8_t *m, size_t mlen,
                  const las_pk *pk, const las_sk *sk, const las_pp *pp);
