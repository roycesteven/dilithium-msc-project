...
1. **Extra computation.** Compared with the basic simplified Dilithium-style signature, LAS adds PreSign (+1.8% over Sign), PreVerify (+7.4% over Verify) and Adapt (+7.7% over Verify), plus Ext, which extracts the witness s = z - ẑ in about 37 microseconds; the adaptor therefore costs roughly one extra signing pass plus a few verification-scale operations.
