...
1. **Extra computation.** Compared with the basic simplified Dilithium-style signature, LAS adds PreSign (+4.3% over Sign), PreVerify (+2.6% over Verify) and Adapt (+5.3% over Verify), plus Ext, which extracts the witness s = z - ẑ in about 38 microseconds; the adaptor therefore costs roughly one extra signing pass plus a few verification-scale operations.
