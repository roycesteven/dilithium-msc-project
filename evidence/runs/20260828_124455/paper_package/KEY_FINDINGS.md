...
1. **Extra computation.** Compared with the basic simplified Dilithium-style signature, LAS adds PreSign (+0.7% over Sign), PreVerify (+2.6% over Verify) and Adapt (+5.2% over Verify), plus Ext, which extracts the witness s = z - ẑ in about 93 microseconds; the adaptor therefore costs roughly one extra signing pass plus a few verification-scale operations.
