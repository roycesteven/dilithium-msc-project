...
1. **Extra computation.** Compared with the basic simplified Dilithium-style signature, LAS adds PreSign (+6.7% over Sign), PreVerify (+3.1% over Verify) and Adapt (+8.1% over Verify), plus Ext, which extracts the witness s = z - ẑ in about 101 microseconds; the adaptor therefore costs roughly one extra signing pass plus a few verification-scale operations.
