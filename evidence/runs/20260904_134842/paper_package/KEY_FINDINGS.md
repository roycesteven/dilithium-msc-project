...
1. **Extra computation.** Compared with the basic simplified Dilithium-style signature, LAS adds PreSign (+2.3% over Sign), PreVerify (+1.6% over Verify) and Adapt (+4.9% over Verify), plus Ext, which extracts the witness s = z - ẑ in about 41 microseconds; the adaptor therefore costs roughly one extra signing pass plus a few verification-scale operations.
