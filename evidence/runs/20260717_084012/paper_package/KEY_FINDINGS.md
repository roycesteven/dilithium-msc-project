...
1. **Extra computation.** Compared with the basic simplified Dilithium-style signature, LAS adds PreSign (+1.6% over Sign), PreVerify (+5.1% over Verify) and Adapt (+9.0% over Verify), plus Ext, which extracts the witness s = z - ẑ in about 84 microseconds; the adaptor therefore costs roughly one extra signing pass plus a few verification-scale operations.

2. **The final signature does not grow.** The basic signature, the pre-signature and the adapted signature have the same 6720 bytes, because Adapt sets z = ẑ + r' (it changes the response value, not the serialized structure).
