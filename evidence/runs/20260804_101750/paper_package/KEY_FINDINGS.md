...
1. **Extra computation.** Compared with the basic simplified Dilithium-style signature, LAS adds PreSign (+6.6% over Sign), PreVerify (+4.0% over Verify) and Adapt (+6.9% over Verify), plus Ext, which extracts the witness s = z - ẑ in about 44 microseconds; the adaptor therefore costs roughly one extra signing pass plus a few verification-scale operations.
