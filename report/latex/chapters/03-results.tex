...
\begin{table}[t]
  ...
  \begin{tabular}{@{}llc@{}}
    ...
    2 & the pre-signature \emph{fails} ordinary Verify (statement-binding tripwire) & pass \\
    3 & Adapt yields a signature that ordinary Verify accepts & pass \\
...
verifiable yet unspendable (the tripwire, clause~2), becomes an ordinary signature
...
overhead of the adaptor layer. \Cref{tab:overhead} reports it at the paper parameters,
pairing each adaptor operation with the base operation it mirrors. Each entry is the
mean $\pm$ sample standard deviation over ten runs of one thousand iterations
(\texttt{bench\_levels}).

\begin{table}[t]
  \centering
  \caption{Primary fair comparison: adaptor overhead over the simplified base, paper
  parameters ($n{=}\ell{=}4$, $\kappa{=}60$), $\mu$s, mean $\pm$ sample SD. Overhead is
  the increase of each adaptor operation over its base analogue.}
  \label{tab:overhead}
  \begin{tabular}{@{}llrr r@{}}
    \toprule
    Adaptor op & paired base op & base ($\mu$s) & adaptor ($\mu$s) & overhead \\
    \midrule
    PreSign   & Sign   & $763 \pm 27$ & $797 \pm 26$ & $+4.4\%$ \\
    PreVerify & Verify & $182 \pm 2$  & $191 \pm 4$  & $+5.0\%$ \\
    Adapt     & Verify & $182 \pm 2$  & $193 \pm 8$  & $+5.9\%$ \\
    Extract   & ---    & ---          & $63 \pm 0.4$ & (separate) \\
    \midrule
    KeyGen / statement gen & & \multicolumn{2}{c}{$80 \pm 1$ (shared)} & --- \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{itemize}
  \item \textbf{PreSign $\approx$ Sign} ($+4.4\%$): PreSign runs the same masked-commit,
  ...
  \item \textbf{PreVerify $\approx$ Verify} ($+5.0\%$): PreVerify recomputes the same
        commitment shape as Verify, differing only by the $+Y$ addition before hashing.
  \item \textbf{Adapt $\approx$ Verify} ($+5.9\%$): Adapt first runs a PreVerify and
        then adds the witness vector, so it costs about one verification plus a few
        polynomial additions.
  \item \textbf{Extract is cheapest and reported separately}: it has no base analogue;
...
measurement at each parameter set: the adaptor premium stays at or below roughly six
percent everywhere, so adding adaptor functionality does not become more expensive as
the scheme is scaled up.

\begin{table}[t]
  \centering
  \caption{Adaptor overhead is small at every parameter set (\texttt{bench\_levels}).
  Percentages are each adaptor operation over its base analogue.}
  \label{tab:overhead-levels}
  \begin{tabular}{@{}lrrrr@{}}
    \toprule
    Set $(n,\ell,\kappa)$ & PreSign/Sign & PreVerify/Verify & Adapt/Verify & Extract ($\mu$s) \\
    \midrule
...
attempts rather than estimating from a timing ratio: at the paper parameters
approximately 37\% of attempts are accepted, so a signature takes about 2.7 attempts
on average. This matches the closed-form prediction: one attempt is accepted only if
...
\begin{table}[t]
  \centering\small
  \caption{LAS objects by component (packed bytes; \texttt{bench\_levels}). The paper
  set shares the NIST~2 dimensions. The challenge $c$ is fixed; the response $z$ grows
  with $n+\ell$ and dominates the signature at every level.}
  ...
  \begin{tabular}{@{}lrrr@{}}
    ...
    \textbf{Signature $(c,z)$} & \textbf{4672} & \textbf{6752} & \textbf{9184} \\
    \quad $z$ as \% of signature & 98.6\% & 99.1\% & 99.3\% \\
    \addlinespace
...
\begin{itemize}
  \item \textbf{The response $z$ is essentially the entire signature} --- 98.6\% at
        NIST~2, rising to 99.3\% at NIST~5. The challenge $c$ is a fixed 64 bytes and
...
preceding section showed the adaptor operations add at most about six percent of
compute over the base scheme; this section shows the objects that move on the wire are
...
For this baseline LAS is instantiated using Dilithium-Level-2-aligned parameters, and
only that level is used. The reason is that secp256k1 offers roughly 128-bit classical
security, and the nearest post-quantum target is NIST level~2 (also pitched at the
128-bit category); aligning LAS to level~2 therefore gives the most like-for-like
security target for the classical scheme. Comparing the 128-bit classical adaptor
against LAS at level~3 or level~5 would pit it against a deliberately stronger
post-quantum configuration and would understate the classical scheme, so the
higher levels are reserved for the parameter-sensitivity study of
\cref{sec:res-compute} and not used here. \Cref{tab:classical} reports both schemes at
their adaptor operations.

\begin{table}[t]
  \centering
  \caption{Classical vs.\ post-quantum adaptor signature: same functionality, different
  security foundation. Classical secp256k1 ($\approx$128-bit classical) and LAS with
  Dilithium-Level-2-aligned parameters. Timings $\mu$s/op; sizes bytes. The statement
  is produced by KeyGen and has the public-key size.}
  \label{tab:classical}
  \begin{tabular}{@{}lrr@{}}
    \toprule
    & ECDSA adaptor & LAS adaptor \\
...
roughly 45--90 times larger than the classical adaptor's, whereas the operation times
stay within the same order of magnitude. Second, the two schemes pay their
...
pre-adaptation signature fails ordinary verification.
