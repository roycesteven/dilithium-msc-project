...
measured on the EVM as a gas-metered contrast. A \emph{complete}, validated native LAS
verifier incurred approximately \gasLasVerifiedM{} million gas for its claim path under
Foundry's \texttt{-{}-gas-report} accounting. Because this figure is not a transaction
receipt, it is not directly comparable with the EIP-7825 per-transaction cap.

% The fit claim is conditional on the registration invariant that no contract checks
% (tab:onchain), and on the measured parameter set, message length, and signature
% instance. Two verifier stages branch on coefficient values, so the single-instance
% scope is load-bearing.

A restructured verifier was subsequently mined by a real client within that cap, at the
target parameter set, for the measured message length and single signature instance, and
only under the registration invariant that \cref{tab:onchain} records as unchecked.
This shows that the native implementation exceeded the practical venue limit, rather than
that LAS verification is inherently impossible there. The venue decision therefore rests
on cost, not impossibility.
...
lattice primitives from ZKNox's ETHDILITHIUM, which its authors describe as experimental
and unaudited, so the gas
