...
Boneh--Lynn--Shacham (BLS) signatures over an elliptic curve. ECDSA and Schnorr rely on the
hardness of the elliptic-curve discrete logarithm problem, while BLS relies on a
Diffie--Hellman-type assumption in pairing-friendly elliptic-curve groups. Shor's algorithm
solves discrete logarithms in polynomial time on a sufficiently large quantum computer
\cite{shor1999polynomial}, breaking all three, so a future quantum adversary could forge
signatures and spend other users' funds. ``Post-quantum'' (PQ) cryptography replaces these assumptions with
