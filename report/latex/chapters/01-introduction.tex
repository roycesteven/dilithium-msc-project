...
The primitive is best understood against the atomic swap's history. Cross-chain exchange
first relied on custodial intermediaries; hash-time-locked contracts removed the
custodian by locking both transfers under one hash preimage, so claiming one leg reveals
the preimage unlocking the other. Such contracts, however, need explicit on-chain script
support, place a common hash on both chains --- linking the legs --- and enlarge the
transactions. The adaptor signature answers all three
...
approach, favouring depth over breadth.
