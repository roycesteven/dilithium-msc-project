...
\textsc{Extract}, which recovers $y = \vecz - \hat{\vecz}$ (\cref{fig:lasfuncs}).
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[pan] (pa) at (ia.south)  {commitment $w$\\[1pt]
    ...
    \node[io]  (ic) at (tc.south)  {in: $Y$, key $t$, $\hat{\sigma}$, $M$, witness $y$};
    \node[pan] (pc) at (ic.south)  {\emph{runs (b) first}\\[1pt]
        $\vecz \leftarrow \hat{\vecz} + y$\\[1pt]
        $\inftynorm{y} \le 1$, so\\
    ...
    \node[pan] (pd) at (id.south)  {$y \leftarrow \vecz - \hat{\vecz}$\\[1pt]
        return $y$ iff $A\,y = Y$\\[1pt]
        \emph{needs only public data}};
    \node[io]  (od) at (pd.south)  {out: the witness $y$};
  \end{tikzpicture}
  ...
  only (c) and (d) touch the witness $y$, and they are the two functions a basic
  signature has no counterpart for. How the four compose, and why publishing $\sigma$ is
  what makes a swap atomic, is \cref{fig:flow}.}
  \label{fig:lasfuncs}
\end{figure}
...
$\inftynorm{y}\le 1$, and because \textsc{PreSign} rejects at $\gamma-\kappa-1$ the response
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
      ...
      >={Stealth[]}]
    \node[box] (ps)  {PreSign\\$\hat{\sigma}=(\tilde c,\hat{\vecz})$};
    \node[box, right=of ps] (pv) {PreVerify};
    \node[box, right=of pv] (ad) {Adapt\\$\vecz=\hat{\vecz}+y$};
    \node[box, right=of ad] (vf) {Verify /\\Publish $\sigma$};
    \node[box, right=of vf] (ex) {Extract\\$y=\vecz-\hat{\vecz}$};
    \draw[->] (ps) -- (pv);
    \draw[->] (pv) -- (ad);
    \draw[->] (ad) -- (vf);
    \draw[->] (vf) -- (ex);
...
clean split: the entire lattice core is reused unchanged, the only modified file is the
\texttt{Makefile}, and the scheme, serialization, application and evaluation are new. No upstream function is modified, so the security-critical arithmetic is exactly
