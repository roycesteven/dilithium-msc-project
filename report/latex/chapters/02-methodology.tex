...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[note] at (55.5mm, 2.5mm) {The paper's random oracle
    ...
    \node[io]  (ic) at (tc.south)  {in: $\hat{\sigma}$, witness $y$};
  ...
  (b) touch the statement $Y$, and only in one place --- the challenge hash --- which is
  why the adaptor layer inherits the base scheme's arithmetic rather than replacing it;
  only (c) and (d) touch the witness $y$, and they are the two functions a basic
  signature has no counterpart for. The bound in (a) is the one quantity that must not be
  relaxed: pre-signing rejects one unit tighter than \cref{eq:signbound} precisely so that
  the addition in (c) cannot carry the adapted response past the bound the base verifier
  enforces. How the four compose, and why publishing $\sigma$ is what makes a swap
  atomic, is \cref{fig:flow}.}
  \label{fig:lasfuncs}
\end{figure}
