...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[pan] (pb) at (ib.south)  {$w' = A\hat{\vecz} - c\,t$\\[1pt]
        accept iff\\
        $c = H(\mathit{pk},\,w'+Y,\,M)$\\[1pt]
...
latter behind a single-file bridge.
...
encoding, a \emph{validating} decoder and a verify-from-bytes entry point were implemented.
Two decisions matter. The decoder is \emph{defensive}: it is the interface an on-chain
verifier would expose, and the object the tamper test attacks. And, following FIPS~204
