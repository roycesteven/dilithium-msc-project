...
contract LASGasBreakdownTest {
    ...
    /// Intrinsic transaction gas: 21,000 plus EIP-2028 calldata pricing. Identical to
    /// `TwoLegSwapGas.t.sol::_intrinsic`.
    ...
    function test_breakdown_and_totals() public view {
        ...
        uint256 intrOld = _intrinsic(cdOld);
        ...
        uint256 intrNew = _intrinsic(cdNew);

        console.log("EIP-7825 per-transaction gas cap", TX_GAS_CAP);
        console.log("");
        console.log("BASELINE  LASVerify (vendored primitives)");
        console.log("  calldata bytes", cdOld.length);
        console.log("  exec / intrinsic / TOTAL", execOld, intrOld, execOld + intrOld);
        console.log("OPTIMISED LASVerifyOpt");
        console.log("  calldata bytes", cdNew.length);
        console.log("  exec / intrinsic / TOTAL", execNew, intrNew, execNew + intrNew);
        console.log("  TOTAL as percent of the cap", ((execNew + intrNew) * 100) / TX_GAS_CAP);
    ...
    function test_optimised_fits_in_one_transaction() public view {
        ...
        uint256 total = execGas + _intrinsic(cd);
        console.log("one-transaction TOTAL (exec + intrinsic)", total);
    ...
    function test_baseline_still_exceeds_the_cap() public view {
        ...
        require(execGas + _intrinsic(cd) > TX_GAS_CAP, "baseline no longer exceeds the cap");
    }
}
...
///
/// `LASGasBreakdownTest` prices the verifier; this prices the whole settlement the way
/// `TwoLegSwapGas.t.sol` prices `claimLASVerified`, so `forge test --gas-report` carries
/// a `claimLASVerifiedOpt` row directly comparable with the `claimLASVerified` row in the
/// same table — same escrow, same storage writes, same transfer, only the verifier
/// differs. `scripts/plot_onchain_gas.py` picks that row up automatically.
contract LasVerifiedOptSwapGas {
    Vm constant vm = Vm(0x7109709ECfa91a80626fF3989D68f67F5b1DD12D);
    uint256 constant N = 256;
    ...
    function test_gas_claim_las_verified_opt() public {
        ...
        bytes32 ctx = keccak256(abi.encode(aHatP, tHatP, tPacked, message));
        uint256 id = swapChain.fundLASVerified{value: COIN}(beneficiary, uint64(block.timestamp + 1 days), ctx);

        bytes memory cd =
            abi.encodeWithSelector(AdaptorSwap.claimLASVerifiedOpt.selector, id, sig, aHatP, tHatP, tPacked, message);
        uint256 before = gasleft();
        ...
        require(execGas + intrinsic < TX_GAS_CAP, "settlement does not fit in one transaction");
